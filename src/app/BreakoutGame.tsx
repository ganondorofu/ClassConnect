...
export function BreakoutGame({ fullscreen = false }: Props) {
  ...
  function update() {
    ...
    for (const block of blocksRef.current) {
      ...
      const oL = b.x + BALL_R - block.x;
      const oR = block.x + block.w - (b.x - BALL_R);
      const oT = b.y + BALL_R - block.y;
      const oB = block.y + BLOCK_H - (b.y - BALL_R);
      if (Math.min(oL, oR) < Math.min(oT, oB)) b.dx = -b.dx;
      else b.dy = -b.dy;
      break;
    }
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      const rect = canvas.getBoundingClientRect();
      const t = e.touches[0];
      const logX = (t.clientX - rect.left) * (LW / rect.width);
      paddleXRef.current = Math.max(0, Math.min(LW - PADDLE_W, logX - PADDLE_W / 2));
    };
